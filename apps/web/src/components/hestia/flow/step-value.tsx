  ...
  const fetchDiscount = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDiscount({ tier: d.tierName || d.tier, discount: d.discountPercent ?? d.discount ?? 0, savings: d.estimatedSavings ?? d.savings ?? 0, parametricTriggered: d.parametricTriggered, source: d.source });
      }
    } catch {
    ...
    <div className="h-full flex items-center justify-center" style={{ background: '#0a0810' }}>
      ...
      <div className="w-full max-w-5xl px-10">
        <div className="flex items-center gap-3 mb-4">
          <span className="text-[12px] font-mono tracking-[0.2em] uppercase" style={{ color: 'rgba(251,146,60,0.5)' }}>Step 7 · Insurance Impact</span>
        ...
        <div className="flex items-end gap-6 mb-6">
          <div>
            ...
            <div className="text-[13px] text-white/60 mt-1" style={{ fontWeight: 300 }}>annual premium savings</div>
          </div>
          <div className="pb-1 flex gap-6">
            <div>
              <div className="text-[12px] text-white/65 uppercase tracking-wider">Tier</div>
              <div className="text-lg font-mono text-orange-400" style={{ fontWeight: 300 }}>{discount?.tier || '...'}</div>
            </div>
            <div>
              <div className="text-[12px] text-white/65 uppercase tracking-wider">Discount</div>
              <div className="text-lg font-mono text-emerald-400" style={{ fontWeight: 300 }}>{discount?.discount || 0}%</div>
            </div>
            <div>
              <div className="text-[12px] text-white/65 uppercase tracking-wider">WRC/Acre</div>
              <div className="text-lg font-mono text-white/65" style={{ fontWeight: 300 }}>{wrcPerAcre}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div className="mb-5">
              <div className="flex gap-1 mb-2">
                {DISCOUNT_TIERS.map((t, i) => (
                  <div key={t.name} className="flex-1">
                    ...
                    <div className="text-[11px] text-center" style={{ color: i === tierIdx ? '#FB923C' : i < tierIdx ? '#059669' : 'rgba(255,255,255,0.15)' }}>
                      {t.name} <span className="text-white/60">{t.discount}%</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="flex items-center justify-between mt-2">
                ...
                {discount && (
                  <span className="text-[12px] font-mono px-1.5 py-0.5" style={{
            ...
            {guidePhase === 1 && !success && (
              <button onClick={advanceGuide} className="w-full py-2.5 mb-4 text-[11px] font-medium text-white/65" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, ...pulse(true) }}>
                Got it
              </button>
            )}
            ...
            {guidePhase >= 2 && (
              <div className="space-y-4">
                {[
                  ...
                  <div key={s.label}>
                    <div className="flex justify-between text-[12px] mb-1"><span className="text-white/55">{s.label}</span><span className="font-mono text-white/60">{s.fmt(s.value)}</span></div>
                ...
                {guidePhase === 2 && !success && (
                  <button onClick={advanceGuide} className="w-full py-2.5 text-[11px] font-medium text-white/65" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, ...pulse(true) }}>Lock Parameters</button>
                )}
              </div>
            )}
          </div>
          ...
          <div>
            <div className="mb-4" style={{ height: 180 }}>
              <div className="text-[12px] text-white/55 uppercase tracking-wider mb-2">Premium Comparison</div>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} barCategoryGap="30%">
                  <XAxis dataKey="name" tick={{ fontSize: 12, fill: 'rgba(255,255,255,0.5)' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: 'rgba(255,255,255,0.4)' }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `$${(v / 1000).toFixed(0)}K`} />
            ...
            {discount && (
              <div className="space-y-2 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.03)' }}>
                <div className="text-[11px] text-white/65 uppercase tracking-wider mb-2">UN SEEA Ecosystem Accounting</div>
                {[
                  ...
                  ['Monetary', `$${(discount.savings || 0).toLocaleString()}/yr premium savings`],
                ].map(([k, v]) => (
        ...
        <div className="mt-5">
          ...
          {success && (
            <div className="flex items-center justify-between py-3 px-4" style={{ background: 'rgba(234,88,12,0.06)', border: '1px solid rgba(234,88,12,0.1)', borderRadius: 8 }}>
              ...
              <a href={state.insurance?.hashScanLink} target="_blank" rel="noopener noreferrer" className="text-[12px] font-mono text-orange-400/50 hover:text-orange-400 flex items-center gap-1">HashScan <ExternalLink size={9} /></a>
            </div>
          )}
        </div>
      </div>
